...
/* Tipos locales: representan un proyecto tal como lo consume esta vista */
interface Proyecto {
  folio: number;
  ...
  id_owner?: string;
  permiso?: string;
  isOwner?: boolean; // marca si el usuario actual es propietario (puede borrar)
}

/* ── Componente principal ──
   Interfaz para listar, filtrar y abrir proyectos; soporta paginación por scroll infinito
*/
export default function ProyectosDashboard() {
  // datos de usuario (cargados de localStorage al montar)
  const [idusuario, setIdUsuario] = useState<string | null>(null);
  const [nombre, setNombre] = useState<string | null>(null);
  const [apellidopaterno, setapellidopaterno] = useState<string | null>(null);

  // lista de proyectos y estado de carga/paginación
  ...
  // refs para evitar stale closures en callbacks / observer
  ...
  // refs para scroll infinito (root es el contenedor interno)
  ...
  // modal y lógica IA (buscar proyectos con prompt)
  ...
  // sincronizar refs cuando cambian los estados correspondientes
  ...
  // limpia filtros y restaura lista original guardada
  ...
  // buscar usando el "agente" backend (prompt -> proyectos)
  const buscarConAgente = async () => {
    ...
    try {
      ...
      // reemplaza la lista por la respuesta del agente y muestra mensaje
      setProyectos(lista);
      setTotalProyectos(lista.length);
      setAiMessage(data.respuesta_agente || "");
      setShowAIModal(false);
      setAiPrompt("");
      // limpiar filtros visuales tras la búsqueda con IA
      setFiltroFolio("");
      setFiltroNombre("");
      setFiltroFecha("");
      setFiltroArea("");
  ...
  // abrir chat del proyecto: setea sessionStorage y navega al dashboard
  ...
  // abrir modal confirmación eliminar (solo si es owner)
  ...
  const cerrarConfirmacionEliminar = () => {
    if (eliminando) return; // evitar cierres durante operación
    setFolioAEliminar(null);
  };

  // confirmar eliminación -> llamada DELETE al backend y actualizar lista
  ...
  // cargar datos usuario desde localStorage al montar
  useEffect(() => {
  ...
  // helper: consulta permiso en backend para determinar si es OWNER
  ...
  /* ── carga inicial de proyectos para el usuario ──
     - obtiene proyectos (skip=0, limit=PAGE_SIZE)
     - consulta permiso OWNER para cada proyecto en paralelo
  */
  useEffect(() => {
    if (!idusuario) return;
    setLoading(true);
  ...
  /* ── cargarMás: función paginación incremental
         - respeta flags (no repetir si ya está cargando o si hay filtros)
  */
  ...
  // mantener referencia actualizada de cargarMas para el observer
  const cargarMasRef = useRef(cargarMas);
  useEffect(() => { cargarMasRef.current = cargarMas; }, [cargarMas]);

  // IntersectionObserver para scroll infinito; root = contenedor scrolleable
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      {
        root: container,   // usar el contenedor (scroll interno)
        threshold: 0.1,
      }
    );
  ...
  }, [loading]); // re-montar cuando termine el loading inicial para asegurar DOM

  // formatea fecha ISO a dd/mm/yyyy (es-MX)
  ...
  // aplicar filtros locales sobre la lista cargada
  ...
  /* ── JSX: layout con sidebar filtros y lista de tarjetas paginadas ── */
  return (
    <div className="flex h-screen w-full bg-gray-100">

      {/* SIDEBAR: filtros, búsqueda IA y avatar */}
      <aside className="flex h-full w-90 flex-shrink-0 flex-col bg-white px-5 py-6 shadow-sm">
        ...
        <div className="mt-auto pt-1 left-0 w-full flex justify-center">
          <img src="/images/banortelogo.png" alt="Logo" className="h-15" />
        </div>
      </aside>

      {/* MAIN: resultados / lista de proyectos */}
      <main className="flex min-h-0 flex-1 flex-col p-6">
        ...
        {loading ? (
          ...
          >
            ...
            {/* Sentinel al final para detectar scroll al final y cargar más */}
